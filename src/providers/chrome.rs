...
pub fn chromium_urls<'a>(
    ...
    let pyl_base64 = downloader.get_text(url)
    ...
    let mut urls: HashMap<&LangId, Vec<SourceUrls>> = HashMap::new();
    ...
    for grds in translation_expectations.other_grds.into_values() {
        ...
        for file in grds.files {
            ...
            let grd_base64 = downloader.get_text(&grd_url)
            ...
            for lang_id in lang_ids {
                ...
                urls.entry(lang_id).or_default()
                    .push(SourceUrls { originals: default_url.clone(), translations: url });
            }
        }
    }
